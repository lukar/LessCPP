...
int main() {
	...
	while (window.isOpen()) {
		...
		if ( !game.game_over() ) {
            ...
            displayText += "Turn: "s + (game.active_side() == White ? "White"s : "Black"s) + "\n"s;
		    ...
		    displayText += (game.winning_side() == White ? "White"s : "Black"s) + " wins"s;
		}
		...
		while (window.pollEvent(event)) {
			...
			if (!game.game_over()) {
				...
				if (event.type == sf::Event::MouseButtonPressed) {
                    for (auto &player : game.active_side() == White? whitePlayers: blackPlayers) {
